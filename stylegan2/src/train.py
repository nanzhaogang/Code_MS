...
def save_image_snapshot(out_dir, cur_nimg, model, grid_z, grid_c, grid_size, concat):
    """
    Save image snapshot.
    ...
    Examples:
        >>> save_image_snapshot(out_dir, cur_nimg, model, grid_z, grid_c, grid_size, concat)
    """
    ...
    for z, c in zip(grid_z, grid_c):
        ...
        image = model.synthesis.construct(ws=ws, noise_mode=1)
        images.append(image)
...
def train(args):
    ...
    generator_ema = Generator(z_dim=512, w_dim=512, c_dim=0,
                              img_resolution=img_res, img_channels=3, batch_size=images.shape[0],
    ...
    if resume_train is not None:
        param_dict = load_checkpoint(os.path.join(resume_train, '-G_ema.ckpt'))
        load_param_into_net(generator_ema, param_dict)

    grid_z = split_gpu(Tensor(np.random.randn(labels.shape[0], generator_ema.z_dim), ms.float32))
    grid_c = split_gpu(Tensor(labels, ms.float32))

    imgs = []
    for z, c in zip(grid_z, grid_c):
        ws = generator_ema.mapping.construct(z=z, c=c)
        img = generator_ema.synthesis.construct(ws=ws, noise_mode=1)
        imgs.append(img)
    ...
    while True:
        for num in range(batch_num):
            ...
            if (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
                save_image_snapshot(out_dir, cur_nimg, generator_ema, grid_z, grid_c, grid_size, concat)
...
def parse_args():
    ...
    args.add_argument('--out_dir', help='path to save the generated images', type=str, default='./out/', metavar='DIR')
    args.add_argument('--gpus', help='number of GPUs to use', type=int, default=1, metavar='INT')
    args.add_argument('--snap', help='snapshot interval', type=int, default=10, metavar='INT')
    args.add_argument('--seed', help='random seed', type=int, default=0, metavar='INT')
    args.add_argument('--img_res', type=int, help='image resolution, ffhq=1024, lsun_wide=512', default=1024)
    args.add_argument('--data_dir', help='training data', type=str, default='../dataset/ffhq.zip', metavar='PATH')
    args.add_argument('--xflips', help='enable dataset x-flips', type=bool, default=False, metavar='BOOL')
    args.add_argument('--total_kimg', help='total training duration', type=int, default=25000, metavar='INT')
    args.add_argument('--batch_size', help='total batch size', type=int, default=2, metavar='INT')
    args.add_argument('--start_over', help='start training from scratch', type=bool, default=False, metavar='BOOL')
    args.add_argument('--resume_train', help='network snapshot ckpt path, ie ./out/network-snapshot-001500',
                      type=str, metavar='PATH')
    args.add_argument('--resume_paper', help='path/directory where you save pre-trained ckpt, ie ./ckpt/ffhq/',
                      type=str, metavar='PATH')
