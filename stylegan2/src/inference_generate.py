...
def parse_args():
    ...
    args.add_argument('--ckpt', default='./ckpt/G_ema.ckpt', help='Network checkpoint')
    args.add_argument('--seeds', type=num_range, default='66,230,389,1518',
                      help='seeds option is required, input_format=85,265,297,849 or 601-605')
    args.add_argument('--truncation_psi', type=float, help='Truncation trick', default=0.5)
    args.add_argument('--num_layers', type=int, help='Number of mapping layers', default=8)
    args.add_argument('--noise_mode', type=int, help='Noise mode, 0=none, 1=const, 2=random', default=1)
    args.add_argument('--img_res', type=int, help='Output image resolution, ffhq=1024, lsun_wide=512', default=1024)
    args.add_argument('--out_dir', type=str, help='Output path', default='./generated_images')
    args.add_argument('--grid_size', type=num_range, help='two integers a,b, curate images in axb grid')
