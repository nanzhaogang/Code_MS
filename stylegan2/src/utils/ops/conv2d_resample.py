...
def conv2d_resample(x, w, f=None, up=1, down=1, padding=0, groups=1,
    ...
    if down > 1 and up == 1:
        x = upfirdn2d.upfirdn2d(x=x, f=f, padding=[px0, px1, py0, py1], flip_filter=flip_filter, conv_info=conv_info)
    ...
    if up > 1:
        ...
        x = upfirdn2d.upfirdn2d(x=x, f=f, padding=[px0 + pxt, px1 + pxt, py0 + pyt, py1 + pyt], gain=up ** 2,
                                flip_filter=flip_filter, conv_info=conv_info)
        if down > 1:
            x = upfirdn2d.upfirdn2d(x=x, f=f, down=down, flip_filter=flip_filter, conv_info=conv_info)
        return x
