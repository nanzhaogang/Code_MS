...
from mindcv.models import Conv2dNormActivation
...
class MobileViTBlock(nn.Cell):
    ...
    def __init__(
        ...
        conv_1x1_out = Conv2dNormActivation(
        ...
        conv_3x3_out = None
        if not no_fusion:
            conv_3x3_out = Conv2dNormActivation(
        ...
        self.local_rep = nn.SequentialCell(OrderedDict([
            ('conv_3x3',
             Conv2dNormActivation(in_channels=in_channels, out_channels=in_channels, kernel_size=conv_ksize, stride=1,
                                  activation=Swish, has_bias=False)),
            ('conv_1x1', Conv2dNormActivation(in_channels=in_channels, out_channels=transformer_dim,
                                              kernel_size=1, stride=1, norm=None, activation=None, has_bias=False)),
        ]))
