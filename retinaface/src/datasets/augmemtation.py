...
def bbox_iof(bbox_a, bbox_b, offset=0):
    "Compute iou value of box a and b"
...
def _choose_candidate(max_trial, image_w, image_h, boxes):
    " Randomly crop image from the origin image, take them as condidates to train the model"
...
def _correct_bbox_by_candidates(candidates, input_w, input_h, flip, boxes, labels, landms, allow_outside_center):
    """
    Calculate correct boxes according to the resized image.
    """
    while candidates:
        if len(candidates) > 1:
