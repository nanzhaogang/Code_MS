...
class FPN(nn.Cell):
    ...
    def __init__(self,
        ...
        if out_channel <= 64:
            leaky = 0.1
        norm_layer = nn.BatchNorm2d
        self.output1 = ConvBNReLU(in_channel * 2, out_channel, kernel_size=1, stride=1, padding=0, groups=1,
                                  norm_layer=norm_layer, leaky=leaky)
        self.output2 = ConvBNReLU(in_channel * 4, out_channel, kernel_size=1, stride=1, padding=0, groups=1,
                                  norm_layer=norm_layer, leaky=leaky)
        self.output3 = ConvBNReLU(in_channel * 8, out_channel, kernel_size=1, stride=1, padding=0, groups=1,
                                  norm_layer=norm_layer, leaky=leaky)
...
class RetinaFace(nn.Cell):
    ...
    def __init__(self, phase='train', backbone=None):
        ...
        if self.backbone == 'resnet50':
        ...
        elif self.backbone == 'mobilenet25':
            ...
            self.ClassHead = self._make_class_head(fpn_num=3, inchannels=[64, 64, 64], anchor_num=[2, 2, 2])
            self.BboxHead = self._make_bbox_head(fpn_num=3, inchannels=[64, 64, 64], anchor_num=[2, 2, 2])
            self.LandmarkHead = self._make_landmark_head(fpn_num=3, inchannels=[64, 64, 64], anchor_num=[2, 2, 2])
        else:
            raise Exception('Backbone only can be resnet50 or mobilenet25')
...
class RetinaFaceWithLossCell(nn.Cell):
    "Combine the retinaface network and loss function"
    def __init__(self, network, multibox_loss):
        super(RetinaFaceWithLossCell, self).__init__()
        self.network = network
        self.loc_weight = 2.0
        self.class_weight = 1.0
        self.landm_weight = 1.0
...
class TrainingWrapper(nn.Cell):
    "Wrap the optimization and network togather"
...
class MobileNetV1(nn.Cell):
    ...
    def __init__(self):
        ...
        self.fc = nn.Dense(in_channels=256, out_channels=1000)
